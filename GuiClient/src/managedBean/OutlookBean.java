...
public class OutlookBean {
	...
	public String loginIdentity() 
	{
		...
		try 
		{
			...
			if(identityDAORemote.hasOutlookAccount(identityDTO))
			{
				return "/userFilter/outlookHome.xhtml?faces-redirect=true";
			}
			else
			{
				facesContext.addMessage("loginForm", new FacesMessage(FacesMessage.SEVERITY_ERROR, "You must have access to Outlook in order to access this page!", null));
				return null;
			}
		} 
